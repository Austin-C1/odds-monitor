...
const ALL_ACCOUNTS_OPTION = '__all_accounts__' as const
...
const PositionList: React.FC = () => {
  ...
  const sellQuantity = Form.useWatch('quantity', form) ?? ''
  ...
  const fetchRedeemableSummary = async () => {
    ...
    try {
      ...
      message.error(error.message || '获取可赎回统计失败')
    } finally {
  ...
  const handleRedeemSubmit = async () => {
    ...
    try {
      ...
      if (error.response?.data?.code === 2014 || error.message?.includes('Builder API Key 未配置')) {
        message.error({
          content: error.response?.data?.msg || error.message || 'Builder API Key 未配置，请前往账户管理页面配置',
          duration: 5,
        })
        // 延迟跳转，让用户看到错误消息
        setTimeout(() => {
  ...
  const handleSellSubmit = async () => {
    ...
    try {
      ...
      const quantity = values.quantity as string | undefined
      const price = values.limitPrice as string | undefined
    ...
    <div>
      <div style={{ marginBottom: '16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
          <Select<number | typeof ALL_ACCOUNTS_OPTION>
            placeholder="选择账户"
            value={selectedAccountId ?? ALL_ACCOUNTS_OPTION}
            onChange={(value: number | typeof ALL_ACCOUNTS_OPTION) => {
              setSelectedAccountId(value === ALL_ACCOUNTS_OPTION ? undefined : Number(value))
            }}
            style={{ width: isMobile ? '100%' : 200 }}
            loading={accountsLoading}
            options={[
              { value: ALL_ACCOUNTS_OPTION, label: '全部账户' },
              ...accounts
                .sort((a, b) => {
          ...
          <div>
            ...
            <div style={{ marginTop: '16px' }}>
              ...
              <Table
                ...
                rowKey={(record) => `${record.accountId}-${record.marketId}-${record.outcomeIndex}-${record.side}`}
                pagination={false}
                size="small"
                scroll={{ y: 300 }}
                columns={[

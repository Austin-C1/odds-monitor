...
class CrownCollector(
    ...
    private val logger = LoggerFactory.getLogger(CrownCollector::class.java)

    fun sourceKey(): String = SOURCE_KEY

    @Transactional
    fun collectOnce(): CrownCollectionResult {
        val config = dataSourceConfigRepository.findBySourceKey(SOURCE_KEY)
        if (config == null || !config.enabled) {
            return CrownCollectionResult("disabled", null, 0)
        }
        ...
        return try {
            ...
            saveLog(
                startedAt = startedAt,
                status = "success",
                message = "collected ${stats.rowCount} crown odds rows",
                recordsCount = stats.rowCount,
                matchCount = stats.matchCount,
                marketCount = stats.rowCount,
                emptyMarketCount = stats.emptyMarketCount
            )
            CrownCollectionResult("success", null, stats.rowCount)
            ...
            saveFailure(startedAt, "failed_runtime", message)
            CrownCollectionResult("failed_runtime", message, 0)
        }
    }

    private fun writeMatches(matches: List<CrownFootballMatch>): WriteStats {
    ...
    private fun savePlatformMatch(match: CrownFootballMatch): OddsPlatformMatch {
    ...
    private fun saveOddsRow(platformMatch: OddsPlatformMatch, standardMatchId: Long, row: CrownMappedOddsRow) {
        val now = System.currentTimeMillis()

...
class SystemConfigController(
    ...
    @PostMapping("/order-notification-min-amount/update")
    fun updateOrderNotificationMinAmount(
        @RequestBody request: OrderNotificationMinAmountUpdateRequest
    ): ResponseEntity<ApiResponse<SystemConfigDto>> {
        return try {
            val minAmount = request.minAmountUsdc
                ?.trim()
                ?.takeIf { it.isNotEmpty() }
                ?.toBigDecimalOrNull()
                ?: return ResponseEntity.ok(
                    ApiResponse.error(ErrorCode.PARAM_ERROR, "minAmountUsdc is required", messageSource)
                )

            val result = systemConfigService.updateOrderNotificationMinAmount(minAmount)
            result.fold(
                onSuccess = { config ->
                    ResponseEntity.ok(ApiResponse.success(config))
                },
                onFailure = { e ->
                    logger.error("Failed to update Telegram order notification minimum amount: ${e.message}", e)
                    ResponseEntity.ok(
                        ApiResponse.error(
                            ErrorCode.SERVER_ERROR,
                            "Failed to update Telegram order notification minimum amount: ${e.message}",
                            messageSource
                        )
                    )
                }

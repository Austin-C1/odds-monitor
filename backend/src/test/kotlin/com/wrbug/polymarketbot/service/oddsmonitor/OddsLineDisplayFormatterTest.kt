...
class OddsLineDisplayFormatterTest {
    ...
    fun `formats quarter handicap ranges for display`() {
        ...
        assertEquals("0/0.5", OddsLineDisplayFormatter.format("handicap", "0.25"))
        assertEquals("0.5/1", OddsLineDisplayFormatter.format("handicap", "0.75"))
        assertEquals("0/-0.5", OddsLineDisplayFormatter.format("handicap", "-0.25"))
        assertEquals("-0.5/-1", OddsLineDisplayFormatter.format("handicap", "-0.75"))
    ...
    fun `keeps totals readable without changing market meaning`() {
        ...
        assertEquals("2.5/3", OddsLineDisplayFormatter.format("total", "2.75"))
    }
